...
def init_contract_field_changes_table():
    """Create the contract_field_changes table (idempotent, SQLite + PG).

    Records field-level diffs between consecutive contract snapshots. The PG
    schema is also defined in migrations/012_contract_field_changes.sql; this
    CREATE TABLE IF NOT EXISTS keeps fresh SQLite installs and re-runs safe.
    """
    with get_engine().begin() as conn:
        conn.execute(text("""
...
def clear_field_changes_for_date(run_date):
    """Remove previously recorded field changes for a run_date (idempotent re-run)."""
...
def insert_field_change(run_date, contract_id, field_name, old_value, new_value,
                        old_snapshot_id=None, new_snapshot_id=None):
    """Record a single field-level change between two contract snapshots."""
    init_contract_field_changes_table()
    with get_engine().begin() as conn:
        conn.execute(text("""
        INSERT INTO contract_field_changes (
            run_date, contract_id, field_name, old_value, new_value,
            old_snapshot_id, new_snapshot_id
        )
        VALUES (:run_date, :contract_id, :field_name, :old_value, :new_value,
                :old_snapshot_id, :new_snapshot_id)
        """), {
            "run_date": run_date,
...
def get_field_changes(run_date):
    """Return all recorded field-level changes for a run_date (oldest first)."""
    init_contract_field_changes_table()
    with get_engine().connect() as conn:
        rows = conn.execute(text("""
            SELECT run_date, contract_id, field_name, old_value, new_value,
                   old_snapshot_id, new_snapshot_id, changed_at
            FROM contract_field_changes
            WHERE run_date = :run_date
            ORDER BY contract_id, field_name
