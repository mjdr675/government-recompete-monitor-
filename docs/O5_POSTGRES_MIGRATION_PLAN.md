...
**Status: COMPLETE.** Postgres + Redis are provisioned in the Railway project. The
web service is live on PostgreSQL 18.4 (cutover 2026-07-10), and the Celery `worker`
and `beat` services have been **created and are running in production** (beat
dispatch verified after the successful PR #71 deploy). Worker/beat service defs and
the shared `DATABASE_URL`/`REDIS_URL` references are active in `railway.toml`. The
human data cutover (steps 2–6 below) and worker/beat activation (step 7) are all
done. This document is retained as the **historical plan of record** and as the
reference for any future rollback/rebuild — it is no longer an outstanding action.
**Goal (achieved):** run Celery `worker` and `beat` in prod so scheduled/async jobs
(watchlist alerts, trial-reminder emails, beat health) actually run. The nightly
ingest is owned solely by the `daily-ingest` cron — it is **not** a beat job.
...
- Before the 2026-07-10 cutover, `web` served off **SQLite** at `/data/contracts.db`,
  on a Railway **volume** (it now runs on shared Postgres — see Status above). This
  section explains why the move to shared Postgres was required.
...
- Therefore all three services must share a **network Postgres** via `DATABASE_URL`.
  ...
  `railway.toml`, and the `worker` and `beat` services have since been **created and
  are running in production** — the former human-only activation runbook is complete
  (see Status above), not a pending gate.
...
> and worker/beat service creation (step 7) is also **complete** — the services are
> running in production (beat dispatch verified after the PR #71 deploy).
...
## Human-only cutover steps (COMPLETED — historical record)

> **These steps have all been completed** (PostgreSQL cutover 2026-07-10 and
> worker/beat activation, verified after the successful PR #71 deploy). They are
> retained as an audit / rollback-rebuild reference and are **no longer current
> operational requirements** — a present-day operator does **not** need to perform
> them, and the imperatives below (e.g. "Do NOT start…", "Do NOT add worker/beat
> yet") describe the original sequence, not present instructions.
>
> The original precondition was: do not start until the SQLite prod DB had a fresh,
> verified off-site backup (the live pre-deploy R2 backup produces one on every web
> start; the latest `PRAGMA integrity_check=ok` + R2 upload was confirmed in the web
> start logs first).
...
7. **Activate worker + beat** — **done**: the two `[[services]]` blocks are active in
   `railway.toml`, and the live `worker` and `beat` Railway services were created per
   DEPLOYMENT.md §8b with `DATABASE_URL` **and** `REDIS_URL` (the
   `${{Postgres.DATABASE_URL}}` / `${{Redis.REDIS_URL}}` references) set on both, then
   deployed.
8. **Verify worker/beat** — **done**: worker running; beat schedule loaded;
   `tasks.heartbeat` and `tasks.check_beat_health` firing; beat dispatch verified
   after the successful PR #71 deploy.
...
- **Premature worker/beat activation** against an empty PG or an empty per-service
  SQLite volume → jobs would act on wrong data. This risk was mitigated during the
  migration by the (now-removed) pre-cutover DO-NOT-DEPLOY gate and by this plan's
  ordering; it **no longer applies** — the cutover is complete and `worker`/`beat`
  were activated intentionally against the populated shared Postgres.
...
## What has been done (repo config + live deployment)
...
- `railway.toml`: `worker`/`beat` service definitions are **active** and, together
  ...
  `${{Redis.REDIS_URL}}`. A `STATUS` comment there records that the worker and beat
  services are now created and running (the pre-cutover `DO NOT MERGE OR DEPLOY`
  prohibition was removed once web went live on Postgres).
...
- Web is **LIVE on PostgreSQL 18.4** (cutover 2026-07-10); the pre-start pg_dump
  backup + R2 upload are verified. The `worker`/`beat` Railway services **have since
  been created and are running in production** (beat dispatch verified after the
  successful PR #71 deploy). The human-only activation runbook has been executed and
  is retained for reference/rollback.
